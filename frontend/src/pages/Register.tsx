...
import { signInWithGoogle, registerWithEmail } from '../firebase/config';
...
const Register: React.FC = () => {
  ...
  const handleEmailRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Register error:', error);
      setError(error.message || 'Gagal mendaftar dengan email. Silakan coba lagi.');
    } finally {
      setIsRegisterLoading(false);
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-blue-500 to-blue-700 relative">
      <div className="bg-white rounded-lg shadow-xl overflow-hidden max-w-4xl w-full flex">
        ...
        <div className="w-full md:w-1/2 p-8 md:p-12">
          <h2 className="text-2xl font-bold mb-8 text-center text-gray-800">Buat Akun</h2>
          ...
          <div className="text-center mb-6">
            <span className="text-sm text-gray-500">atau gunakan email anda untuk registrasi</span>
          </div>
          ...
          <div className="mt-6 text-center md:hidden">
            <p className="text-sm text-gray-600">
              Sudah punya akun? <Link to="/login" className="text-primary hover:text-blue-800 font-medium cursor-pointer" onClick={() => navigate('/login')}>Login sekarang</Link>
            </p>
          </div>
        </div>
        
        {/* Right side - Blue section with welcome message */}
        <div className="hidden md:block md:w-1/2 bg-gradient-to-br from-primary to-blue-600 p-12 text-white relative">
          ...
          <div className="relative z-10">
            ...
            <p className="mb-8">Untuk tetap terhubung dengan kami, silahkan masuk dengan akun anda</p>
            <Link to="/login" className="inline-block border-2 border-white text-white px-6 py-3 rounded-full hover:bg-white hover:text-primary transition-all duration-300 font-medium cursor-pointer" onClick={() => navigate('/login')}>
              SIGN IN
            </Link>
          </div>
        </div>
